	...
	function getComm($number, $order, $id_post) {
		...
		<?php
			while ($donnees = $req->fetch()) {?>
				<div id='Commentaire'>
					<?php echo '<a style="font-weight: bold;" href=\'users.php?id=' . $user->getPseudoByID($donnees['poster']) . '\'>' . $user->getPseudoByID($donnees['poster']) . '</a> : ' . $donnees['comment'] . '<br /><span class="sign" style="text-align: right;">Envoyé par <a href=\'users.php?id=' . $user->getPseudoByID($donnees['poster']) . '\'>' . $user->getPseudoByID($donnees['poster']) . '</a> le ' . $donnees['date'] . '</span>'; ?>
				</div><?php } ?>
		</div></div>
		<?php
	}
